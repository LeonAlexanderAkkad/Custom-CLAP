...
def eval_retrieval(model: Clap, eval_loader: DataLoader) -> dict[str, float]:
    ...
    with torch.no_grad():
        ...
        for i, (_, _, audio_batch) in tqdm(enumerate(eval_loader), total=len(eval_loader), desc="Computing similarity scores"):

            # Compute similarity matrix
            audio_embedding = model.get_audio_embeddings(audio_batch)
